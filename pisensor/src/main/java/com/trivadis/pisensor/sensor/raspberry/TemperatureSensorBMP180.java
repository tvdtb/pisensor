 ...
 * 
 * 
 * https://github.com/OlivierLD/raspberry-pi4j-samples/blob/master/AdafruitI2C/src/adafruiti2c/sensor/AdafruitBMP180.java
 */
public class TemperatureSensorBMP180 implements TemperatureSensor {
	...
	// BMP180 Registers
	public final static int BMP180_CAL_AC1 = 0xAA; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_AC2 = 0xAC; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_AC3 = 0xAE; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_AC4 = 0xB0; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_AC5 = 0xB2; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_AC6 = 0xB4; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_B1 = 0xB6; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_B2 = 0xB8; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_MB = 0xBA; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_MC = 0xBC; // R Calibration data (16
													// bits)
	public final static int BMP180_CAL_MD = 0xBE; // R Calibration data (16
													// bits)
	...
	public TemperatureSensorBMP180() {
		this(BMP180_ADDRESS);
	}

	public TemperatureSensorBMP180(int address) {
		try {
			// Get i2c bus
	...
	private int readU8(int reg) throws Exception {
		...
		try {
			...
			if (verbose)
				System.out.println("I2C: Device " + BMP180_ADDRESS
	...
	private int readU16(int register) throws Exception {
		...
		return (BMP180_ENDIANNESS == BIG_ENDIAN) ? (hi << 8) + lo : (lo << 8)
				+ hi; // Big Endian
	}
	...
	public int readRawPressure() throws Exception {
		...
		bmp180.write(BMP180_CONTROL,
				(byte) (BMP180_READPRESSURECMD + (this.mode << 6)));
		...
		if (verbose)
			System.out.println("DBG: Raw Pressure: " + (raw & 0xFFFF) + ", "
					+ raw);
		return raw;
	}
	...
	public float readPressure() throws Exception {
		...
		if (verbose) {
			...
			System.out.println("DBG: True Temperature = "
					+ (((B5 + 8) >> 4) / 10.0) + " C");
		}
	...
	public double readAltitude() throws Exception {
		...
		altitude = 44330.0 * (1.0 - Math.pow(pressure
				/ standardSeaLevelPressure, 0.1903));
		if (verbose)
	...
	public static void main(String[] args) {
		...
		TemperatureSensorBMP180 sensor = new TemperatureSensorBMP180();
		float press = 0;
		float temp = 0;
		double alt = 0;
		...
		sensor.setStandardSeaLevelPressure((int) press); // As we ARE at the sea
															// level (in San
															// Francisco).
		try {
	...
	public TemperatureEvent readTemperatureFromDevice() {
		try {
			TemperatureEvent result = new TemperatureEvent(LocalDateTime.now(), readTemperature(), readPressure());
			return result;
		} catch (Exception e) {
